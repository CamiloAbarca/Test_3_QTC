<template>
  <v-container grid-list-md>
    <v-layout col wrap>
      <v-flex>
        <v-card v-for="article, index in articlesList"
        :key="article.index">
          <v-card-title>
            {{ article.title }}
          </v-card-title>
          ...
          <v-card-actions>
            <v-btn icon @click="btnEditArticle(index, article.title, article.content, article.fav)">
...
export default {
  ...
  methods: {
    ...
    async btnEditArticle(index, title, content, fav) {
      this.title = title
    ...
    async updateArticle () {
      const list = [...this.articlesList];
      list.splice(this.index, 1, {title: this.title, content: this.content, fav: this.fav})
      this.dialog = false
      this.$store.commit(ARTICLES_LIST, list);
    }
  },
